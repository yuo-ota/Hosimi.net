...
- 併せて、バックエンドの恒星時の計算式・レスポンスのキー名・呼び出し先の定数名にも誤りがあり、
  エンドポイント自体が常に 500 を返す状態だった。
- → `StarField` の `<group>` を観測地と時刻に応じて回転させるようにした。
  恒星時は UTC のみの関数として閉じているため、`src/utils/celestialSphere.ts` で
  端末の時計から直接求めることにし、`GET /api/equatorialCoords` は廃止した。
...
| 項目 | 状態 |
|---|---|
| 8-1 観測地が反映されていない | **対応済み**（天球の回転を実装、赤道座標APIは廃止） |
